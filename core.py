...
class FaceProcessingThread(QThread):
    ...
    def enablePanalarm(self, coreUI):
        if coreUI.panalarmCheckBox.isChecked():
            ...
            coreUI.statusBar().showMessage('活体检测：开启')
        else:
            self.isPanalarmEnabled = False
            coreUI.statusBar().showMessage('活体检测：关闭')
    ...
    def run(self):
        ...
        text_dict={3:"Please watch the camera:",
                    0:"Please wink eyes",
                    1:"Please open mouth",
                    2:"Please raise chin",
                    4:"Detect more than one people"}
            ...
            if CoreUI.cap.isOpened():
                ...
                if self.isPanalarmEnabled:
                    for (face_x, face_y, face_w, face_h) in faces:
                        ...
                        eye_results = eyesCascade.detectMultiScale(roi_img, 1.1, 20)     
                        nose_results=noseCascade.detectMultiScale(roi_img,1.1,4) 
                        mouth_results = mouthCascade.detectMultiScale(roi_img, 1.1, 20)
                        # for (x, y, w, h) in eye_results:
                        #     cv2.rectangle(roi_img, (x,y), (x+w, y+h), color_dict['red'], 2)
                        #     cv2.putText(roi_img, "eye", (x, y-4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict['red'], 1, cv2.LINE_AA)
                        # for (x, y, w, h) in nose_results:
                        #     cv2.rectangle(roi_img, (x,y), (x+w, y+h), color_dict['green'], 2)
                        #     cv2.putText(roi_img, "nose", (x, y-4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict['green'], 1, cv2.LINE_AA)
                        # for (x, y, w, h) in mouth_results:
                        #     cv2.rectangle(roi_img, (x,y), (x+w, y+h), color_dict['blue'], 2)
                        #     cv2.putText(roi_img, "mouth", (x, y-4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict['blue'], 1, cv2.LINE_AA)
                        # cv2.rectangle(img, (face_x,face_y), (face_x+face_w, face_y+face_h), color_dict['white'], 2)
                        # cv2.putText(img, "face", (face_x, face_y-4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict['white'], 1, cv2.LINE_AA)
                        # print("nose number",len(nose_results))
                        # print("eye number:",len(eye_results))
                        # print("mouth number",len(mouth_results))
                    if len(faces)==1:
                        result=[len(eye_results),len(nose_results),len(mouth_results)]
                    else:
                        if len(faces)==0:#no face detect
                            result=[0]
                        else:
                            result=[0,0]
                    
                    if len(result)==1 and up_head_state!=1:
                        if down_head_state==1 and action_check==2 and frameCounter-check_frame>10:
                            up_head_state=1
                            CoreUI.logQueue.put('Info：抬头检测完成！')
                            action_check+=1
                        cv2.putText(frame, text_dict[3], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict["white"], 1, cv2.LINE_AA)
                    if len(result)==2 and up_head_state!=1:
                        cv2.putText(frame, text_dict[4], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict["white"], 1, cv2.LINE_AA)
                    if len(result)==3 and up_head_state!=1:
                        if action_check==0:
                            if result[0]>0 and result[1]>0 and result[2]>0:
                            #if result[1]>=0:
                                if open_eye_state==0:
                                    check_frame=frameCounter
                                open_eye_state=1
                            else:
                                if open_eye_state==1 and frameCounter-check_frame>20:
                                    if result[0]==0 and result[1]>=0 and result[2]>0:
                                    #if result[1]==0:
                                        close_eye_state=1
                                        CoreUI.logQueue.put('Info：眨眼检测完成！')
                                        action_check+=1
                                # else:
                                #     cv2.putText(img, text_dict[3], (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 1, cv2.LINE_AA)
                            cv2.putText(frame, text_dict[action_check], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict["white"], 1, cv2.LINE_AA)
                        if action_check==1:
                            #if result[0]>=0 and result[1]>0 and result[2]>0:
                            # if result[2]>0:
                            #     if close_mouth_state==0:
                            #         check_frame=frameCounter
                            #     close_mouth_state=1              
                            # else:
                            #     if close_mouth_state==1 and frameCounter-check_frame>20:
                            #         #if result[0]>=0 and result[1]>=0 and result[2]>=0:
                            #         # if result[2]>=0:
                            #         #     open_mouth_state=1
                            #         #     CoreUI.logQueue.put('Info：张嘴检测完成！')
                            #         #     action_check+=1
                            #         open_mouth_state=1
                            #         CoreUI.logQueue.put('Info：张嘴检测完成！')
                            #         action_check+=1
                        ...
                        if action_check==2:
                            ...
                            cv2.putText(frame, text_dict[action_check], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_dict["white"], 1, cv2.LINE_AA)
                    if up_head_state==1:
                        #cv2.putText(frame, "All done...", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 1, cv2.LINE_AA)
                        self.isPanalarmEnabled=False
                        #self.activeDetect=True
                        #if self.activeDetect:
                        CoreUI.logQueue.put('Info：活体检测完成！')
                        #QMessageBox.information(self,"Info","活体检测完成",QMessageBox.Ok)
                ...
                if self.isFaceTrackerEnabled:
                    ...
                    for fid in faceTrackers.keys():
                        ...
                        cv2.putText(realTimeFrame, 'tracking...', (10, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255),
                                    2)
